...
/// Parse a pair of floating-point number separated by a comma as a complex
// number.
...
/// `pixel` is a (column, row) pair indicaing a particular pixel in that image.
...
fn test_pixel_to_point() {
    assert_eq!(
        pixel_to_point(
            (100, 200), (25, 175), Complex{ re: -1.0, im: 1.0}, Complex{ re: 1.0, im: -1.0}
        ),
        Complex { re: -0.5, im: -0.75}
    );
}

/// Render a rectangle of the Mandelbrot set into a buffer of pixeles.
/// 
/// The `bounds` argument gives the width and height of the buffer `pixels`,
/// wich holds one grayscale pixel per byte. The `upper_left` and `lower_right`
